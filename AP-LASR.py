...
Directory_Structure = '''The program will make the following directory structure, where ROOT is the directory the script is placed in:
    ...
    '''
Help_String = '''This software automates the process of generating combinatorial protien libraries from ancestral sequence reconstruction.
This software can run in one of four modes.
     ...
     ASR mode will take in a user sequence and conduct ASR, then generate combintorial protein libraries of ancestral protiens. Command-line parameters should be used to specify the directory where output is stored and the executables for CD-Hit, MAFFT, and IQ-Tree. The desired final dataset size can be specified, and sequence supplimentation can be turned on here too.
     ...
     RemakeLibrareies mode will generate new combinatorial libraries from ancestral protiens of a previous ASR run with a specified threshold confidence. This will allow generation of a library with a different size than what was generated with the default threshold confidences. Specify the directory where the ASR results of interest are stored.
...
def fasta2dict(fasta_path, return_dict={}):
    ...
    for line in fastafile:  # For every line in the input fasta
        if line[0] == ">":  # Check if it's a name
            if (
                working_sequence != ""
            ):  # If we already have a working sequence, another name indicates we're done. Otherwise record the name
            ...
            if not all(
                [char for char in working_sequence if (char.isalpha() or char == "-")]
            ):
                raise ValueError(
...
def Is_Valid_AA(AA):  # Is the argurment a valid amino acid or list of amino acids
    if isinstance(AA, str):  # This block lets us evaluate strings of amino acids
        return all(
            [True for residue in AA if ((residue in AA_key) or (residue == "-"))]
        )
    if isinstance(AA, list) and isinstance(
        AA[0], str
    ):  # This block lets us evaluate lists of amino acids
        return all((i in AA_key) or (i == "-") for i in AA)
    if isinstance(AA, list) and isinstance(
        AA[0], list
    ):  # This block lets us evaluate lists of lists of amino acids
        return all(all(((i in AA_key) or (i == "-")) for i in lst) for lst in AA)
    else:
...
def NCBI_to_XML(dirname, sequence, hits=2000, expect_value=0.30, seq_name=""):
    ...
    blast_result_handle = NCBIWWW.qblast(
        "blastp", "nr", sequence, hitlist_size=hits, expect=expect_value
    )
...
def Parse_BlastP_XML(dirname, blastp_xml, sequence, sequence_name=None):
        ...
        if isinstance(sequence, str):
            ...
            with open(f"{dirname}/{return_name}", "a+") as blastp_file:
                for key, F_D_Sequence in Fasta_Dict.items():
                    ...
                    # (len(Sequence)>((1-length_cutoff)*User_Sequence_Length)):
                    if (
                        len(F_D_Sequence) > 0.5 * len(sequence)
                        and len(F_D_Sequence) < 1.5 * len(sequence)
                        and Is_Valid_AA(F_D_Sequence)
                    ):
            ...
            for xml in blastp_xml:
                with open(f"./{dirname}/HitsInfo.csv", "a+") as fout:
                    ...
                    for hit in xml.findall(
                        ...
                        hitid = (hit.find("Hit_id")).text
                        hitdef = (hit.find("Hit_def")).text
                        hitaccession = (hit.find("Hit_accession")).text.replace(
                            ".", "_"
                        )
                        seq = (hit.find("Hit_hsps/Hsp/Hsp_hseq")).text
                        # If the sequence doesn't have unknowns amino acids (annoying) then record it.
                ...
                with open(f"{dirname}/{return_name}", "a+") as blastp_file:
                    for key, F_D_Sequence in Fasta_Dict.items():
                        ...
                        # (len(Sequence)>((1-length_cutoff)*User_Sequence_Length)):
                        if (
                            len(F_D_Sequence) < 10000
                            and len(F_D_Sequence) > 10
                            and Is_Valid_AA(F_D_Sequence)
                        ):
                            # We remove all gaps, because CD-Hit cannot handle
                            # gaps.
    ...
    # searches so our output directory structure should be different.
        ...
        with open(
            f"{dirname}/{sequence_name}_Suppliment.csv", "a+"
        ) as fout:  # DIFFERENT FROM ABOVE
            fout.write(f"Hit ID,Hit Description,Hit Sequence\n")
            # Parsing the XML object, looking for hits
            for hit in blastp_xml.findall(
                "./BlastOutput_iterations/Iteration/Iteration_hits/Hit"
...
def BlastP(dirname, sequence, hits=2000, expect_value=0.2, sequence_name=None):
    ...
    if isinstance(sequence, str):
        ...
        if bool(
            [char for char in sequence if (char not in AA_key)]
    ...
    try:
        ...
        if isinstance(sequence, str):
            ...
            for key, item in sequence.items():
                if not os.path.exists(f"{dirname}/BlastP_XML_{key.replace('.','_')}"):
                    xmls.append(
                        NCBI_to_XML(
                            dirname, item, hits, expect_value, key.replace(".", "_")
                        )
                    )
                else:
                    xmls.append(
    ...
    try:
        ...
        if sequence_name is None and isinstance(
            sequence, str
        ):  # A primary search with one user sequence
...
def Sequence_Processing(dirname, finname, sequence):
    ...
    if isinstance(sequence, dict):
        ...
        Hamming_Dict = Fasta_Dict_Hamming(Fasta_Dict, Fasta_Dict["User_Sequence"])
        for key, item in Hamming_Dict.items():
            # If a given sequence has less than 50% similarity with the user
            # sequence, remove it.
            if (
                item / len(Fasta_Dict["User_Sequence"])
    ...
    if Suppliment_Cutoff != 0:  # If we are doing supplimentation
        # This one line suppliments poorly supported areas of the tree, and
        # updates the dictionary with those additional sequences.
        Fasta_Dict.update(
            fasta2dict(f"{dirname}/{Supplement_Sequences(dirname,Fasta_Dict)}")
        )
    # Save the Supplimented Fasta_Dict
    dict2fasta(Fasta_Dict, f"{dirname}/pre-Alignment.fasta")
    try:
        # Align the supplimented sequences - called the Raw alignment
        os.system(
    ...
    if isinstance(sequence, dict):
        ...
        # ready for IQTree
        user_seq_name = [
            name for name in sequence.keys() if name in Fasta_Dict_Aligned.keys()
        ][0]
        return_name = Post_MAFFT_processing(dirname, Fasta_Dict_Aligned, user_seq_name)
    else:  # If only one
...
# are not removed
def CDHit(dirname, fin, fout, cutoff, suppliment=False):
    try:
...
def Remove_Duplicate_Sequences_FASTA(dirname, fpath, suppliment=False):
    CDHit(dirname, fpath, f"{fpath[:-6]}_temp.fasta", 0.99, suppliment)
...
def Supplement_Sequences(dirname, fasta_dict):
    if not (
        os.path.isdir(f"{dirname}/Sequence_Supplement")
    ...
    # would benefit from supplimenting.
    # Be sure there are no gaps in this dictionary, as CD-Hit will reject
    # those.
    dict2fasta(
        fasta_dict, f"{dirname}/Sequence_Supplement/Sequences_to_be_Supplimented.fasta"
    )
    sequences_list_for_search = []
    CDHit(
        dirname,
        "Sequence_Supplement/Sequences_to_be_Supplimented.fasta",
        "Sequence_Supplement/SingleClusters_Supplemented.fasta",
        Suppliment_Cutoff,
        True,
    )  # Run CD-Hit with supplimentation cutoff to identify 'loner' sequences
    # Looking at the CD-Hit clstr file (which holds cluster information)
    with open(
        f"{dirname}/Sequence_Supplement/SingleClusters_Supplemented.fasta.clstr", "r"
    ) as fin:
    ...
    # be supplimented, and write all sequences together as one file.
    ...
    with open(
        f"{dirname}/Sequence_Supplement/Supplemented_BlastP_Sequences.fasta", "a+"
    ) as fout:
...
def Remove_Insertions(
    fasta_dict, User_Sequence, deletion_percentage=0.02, termini_length=0.05
):
    ...
    user_gap_pos = [
        i for i in range(num_pos) if User_Sequence.startswith("-", i)
    ]  # record all positions with a gap in the user sequence
    ...
    key_sequence_gap_list = [
        (key, [i for i in range(num_pos) if not seq.startswith("-", i)])
    ...
    for key, no_gap_list in key_sequence_gap_list:  # Now, for all sequences
        ...
        for pos in no_gap_list:  # Evaluate the positions where it has amino acids
            if (
                (pos in user_gap_pos)
                and (pos > num_pos * termini_length)
                and (pos < num_pos * (1 - termini_length))
...
def Remove_Deletions(
    ...
    user_gap_pos = [
        i for i in range(num_pos) if User_Sequence.startswith("-", i)
    ]  # record all positions with a gap
    # The object 'key_sequence_list' contains a list of tuples with the name
    # of the sequence, the sequence, and a list of all positions with a gap,
    # in that order.
    # Sorry for the mess of a line, but it prevents unessecary looping.
    key_sequence_gap_list = [
        (key, [i for i in range(num_pos) if seq.startswith("-", i)])
        for key, seq in fasta_dict.items()
    ]
    keys_to_pop = []
    # The ax is already at the root of the trees, and every tree that does not
...
def Clean_all_gaps(fasta_dict):
    ...
    for key, sequence in fasta_dict.items():  # Remove all the gaps
        fasta_dict.update(
            {
                key: "".join(
                    [char for i, char in enumerate(sequence) if i in pos_to_leave]
                )
            }
        )
...
def Post_MAFFT_processing(
    dirname, fasta_dict, multisequence, dynamic_sequence_reduction=True
):
...
def IQTree_ASR(dirname, finname):  # Using the tree/model from the phylogney, do the ASR
    ...
    try:
        ...
        os.system(
            f"{IQTREE_Executable} -s {dirname}/{finname} -te {dirname}/IQTree_Phylo/Phylo.treefile -pre {dirname}/IQTree_ASR/ASR -asr -m {model} -redo -nt AUTO"
        )
    except BaseException:
...
def Select_Ancestor_Nodes(dirname):
    ...
    confident_nodes = (
        []
    )  # these will be the nodes with good values. This isn't quite the same value between *.contree and *.trefile, but close
    ...
    for i in range(len(ASRnodes) - 1):
        try:
            ...
            if (SHALRT_i > 80) and (
                UFB_i > 95
            ):  # If the SH-aLRT >80% and the ultrafast bootstraping is >95%
...
def Binary_Gap_Analysis(dirname, finname):
    ...
    for key, seq in binary_dict.items():  # Make a binary alignment of the input fasta
        ...
        binary_dict.update({key: ("".join(["0" if aa == "-" else "1" for aa in seq]))})
    ...
    try:
        os.system(
            f"{IQTREE_Executable} -s {dirname}/IQTree_Binary/Binary_Alignment.fasta -te {dirname}/IQTree_Phylo/Phylo.treefile -pre {dirname}/IQTree_Binary/Binary -blfix -asr -m GTR2+FO -redo -nt AUTO"
        )
    except BaseException:
    ...
    dict2fasta(
        Consensus_Ancestors_with_Gaps, f"{dirname}/Consensus_Ancestors_with_Gaps.fasta"
    )
...
def Make_Uncertianty_Libraries(
    dirname, ASR_Statefile_Dict, Binary_Statefile_Dict, Cutoff=0
):
    ...
    if Cutoff == 0:
        ...
        for cutoff_value in [0.25, 0.2, 0.15, 0.125, 0.1, 0.075, 0.05, 0.025]:
            ...
            for node in Good_Ancestor_Nodes:
                ...
                for i, pos in enumerate(Positions):  # For every position,
                    ...
                    if (Binary_Statefile_Dict[node][i][0]) < 0.5:
                        ...
                        for j, prob in enumerate(
                            pos
                        ):  # For every probability at that position,
                            if (
                                prob
                            ) > cutoff_value:  # If the amino acid is above the threshold
                                pos_AAs.append(
                                    AA_key[j]
                                )  # Record that AA at that position
                ...
                if node in Degen_Seqs_Size:  # If there's already an object
                    Degen_Seqs_Size[node].append(Library_Size_Count(Degenerate_DNA))
                else:  # If we need to intialize the list for the node
                    Degen_Seqs_Size[node] = [Library_Size_Count(Degenerate_DNA)]
                with open(
                    f"{dirname}/DNA_Libraries/Library_{cutoff_value*100}%_Cutoff.fasta",
                    "a+",
                ) as fout:
                    fout.write(f">{node}\n{Degenerate_DNA}\n")
        with open(
            f"{dirname}/DNA_Libraries/Library_Size_Information.csv", "w+"
        ) as fout:
            fout.write(f"Confidece Threshold,{','.join(Good_Ancestor_Nodes)},\n")
            for i, value in enumerate(
                [0.25, 0.2, 0.15, 0.125, 0.1, 0.075, 0.05, 0.025]
            ):
                row_list = [
                    str(Degen_Seqs_Size[node][i]) for node in Good_Ancestor_Nodes
                ]
                fout.write(
        ...
        for node in Good_Ancestor_Nodes:
            ...
            with open(
                f"{dirname}/DNA_Libraries/Library_{Cutoff*100}%_Cutoff.fasta", "a+"
            ) as fout:
                fout.write(f">{node}\n{Degenerate_DNA}\n")
        # This will append a new line to the bottom of the .csv containing the
        # sizes for the library with the new treshold.
        with open(
            f"{dirname}/DNA_Libraries/Library_Size_Information.csv", "a+"
        ) as fout:
            fout.write(
...
def Write_Confidences(dirname, ASR_Statefile_Dict, Binary_Statefile_Dict):
    ...
    for (
        ...
        for i, pos in enumerate(
            cons_list
        ):  # pos is AA distribution the position i of ancestor at node node
        ...
        nodes_data[node] = (
            (sum(node_confidences) / len(node_confidences)),
    ...
    with open(f"{dirname}/IQTree_ASR/Ancestral_Sequence_Confidences.txt", "w+") as fout:
        fout.write(
            f"For the ASR in {dirname} overall:\n\
...
if __name__ == "__main__":
    ...
    parser_mode1 = subparsers.add_parser(
        "Assist", help="Get assistance in running AP-LASR."
    )  # Help mode
    ...
    parser_mode3.add_argument(
        "-n", "--name", action="store", dest="directory", default="ASR"
    )
    ...
    parser_mode4.add_argument(
        "-t", "--threshold", action="store", dest="threshold", required=True, type=float
    )
    # parser.set_defaults(func=lambda args: parser.print_help())
    args = parser.parse_args()
    if (
        (args.mode == "ASR")
        or (args.mode == "MakeFigures")
        or (args.mode == "RemakeLibraries")
    ):
    ...
    if args.mode == "ASR":
        ...
        if not (Final_Dataset_Size > 50):
            print(
                "User provided value for the final dataset size was too small. Preceding with a final dataset size of 60 sequences."
            )
        ...
        with open(f"{directory}/{Final_Name}") as fin:
            if len(fin.readlines()) < 80:
                print(
                    "WARNING: There are very few sequences in the final sequence alignment. This can indicate many highly similar sequences were returned from BlastP or that there are not many sequences available."
                )
    ...
    if args.mode == "MakeFigures":
        ...
        for (
            ...
            for i, pos in enumerate(
                cons_list
            ):  # pos is the position of ancestor at node
        ...
        for node, item in Consensus_Ancestors_with_Gaps_and_Confidence.items():
            seq_heatmap(
                {node: item}, f"{directory}/Confidence_Heatmaps/{node}_Confidences.pdf"
            )
